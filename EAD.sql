...
BEGIN
	TABELA_USUARIO := RETORNAR_TABELA_DO_USUARIO();
	IF TABELA_USUARIO != 'PROFESSOR' THEN
		RAISE EXCEPTION 'É OBRIGATÓRIO ESTAR LOGADO COMO PROFESSOR PARA DELETAR UM CURSO!';
	ELSE
...
-- SELECT * FROM CURSO
-- SELECT FROM PROFESSOR_PUBLICAR_CURSO(3)
...
BEGIN
	...
	ELSE
		RAISE EXCEPTION 'NÃO É PERMITIDO QUE UM PROFESSOR CRIE UM MÓDULO NO CURSO DE OUTRO!';
	END IF;
END
$$ LANGUAGE plpgsql;
