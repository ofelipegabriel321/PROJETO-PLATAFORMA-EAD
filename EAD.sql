...
BEGIN
	...
	ELSIF TABELA = 'MODULO' THEN
		IF VERIFICAR_SE_REGISTRO_EXISTE(CODIGO_TABELA_VERIFICADA, 'MODULO') IS FALSE THEN
			RAISE EXCEPTION 'ESSE MÓDULO NÃO EXISTE, DIGITE UM COD_MODULO VALIDO!';
	...
	ELSIF TABELA = 'DISCIPLINA' THEN
		IF VERIFICAR_SE_REGISTRO_EXISTE(CODIGO_TABELA_VERIFICADA, 'DISCIPLINA') IS FALSE THEN
			RAISE EXCEPTION 'ESSA DISCIPLINA NÃO EXISTE, DIGITE UM COD_DISCIPLINA VALIDO!';
	...
	ELSIF TABELA = 'VIDEO_AULA' THEN
		IF VERIFICAR_SE_REGISTRO_EXISTE(CODIGO_TABELA_VERIFICADA, 'VIDEO_AULA') IS FALSE THEN
			RAISE EXCEPTION 'ESSA VIDEO_AULA NÃO EXISTE, DIGITE UM COD_VIDEO_AULA VALIDO!';
...
BEGIN
	IF VERIFICAR_SE_REGISTRO_EXISTE(CODIGO_MODULO, 'MODULO') IS FALSE THEN
		RAISE EXCEPTION 'ESSE MODULO NÃO EXISTE, DIGITE UM COD_MODULO VALIDO!';
	ELSE
...
BEGIN
	...
	ELSIF VERIFICAR_SE_REGISTRO_EXISTE(CODIGO_MODULO_PRE_REQUISITO, 'MODULO') IS FALSE THEN
		RAISE EXCEPTION 'ESSE MODULO NÃO EXISTE, DIGITE UM COD_MODULO_PRE_REQUISITO VALIDO!';
	ELSE
...
/* ASSISTIR VIDEOS */
CREATE OR REPLACE FUNCTION ASSISTIR_VIDEO_AULA(CODIGO_ALUNO TEXT, CODIGO_VIDEO_AULA INT)
RETURNS VOID
AS $$
DECLARE
	...
	CODIGO_CURSO INT := (SELECT COD_CURSO FROM CURSO C_S INNER JOIN MODULO M_D ON C_R.COD_CURSO = M_D.COD_CURSO
			INNER JOIN DISCIPLINA D_C ON M_D.COD_MODULO = D_C.COD_MODULO INNER JOIN VIDEO_AULA V_A ON
			COD_VIDEO_AULA = CODIGO_VIDEO_AULA);
BEGIN
		...
		ELSE
			...
			ELSE
				IF ALUNO_AINDA_CURSANDO(CODIGO_ALUNO, CODIGO_CURSO) IS FALSE THEN
					RAISE EXCEPTION 'ESSE ALUNO NÃO ESTÁ MAIS CURSANDO O CURSO DESSA VIDEO_AULA!';
				ELSE
					IF ALUNO_JA_ASSISTIU(CODIGO_ALUNO, CODIGO_VIDEO_AULA) = FALSE THEN
...
BEGIN
	...
	IF VERIFICAR_PERMISSAO_DO_USUARIO(CODIGO_CURSO, 'CURSO') IS TRUE THEN
		PERFORM CRIAR_MODULO(CODIGO_CURSO);
	ELSE
...
/* ASSISTIR VIDEOS */
-- OBS: FALTA VERIFICAR SE O ALUNO AINDA ESTA CURSANDO?? -- <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
CREATE OR REPLACE FUNCTION ASSISTIR_VIDEO_AULA(CODIGO_ALUNO TEXT, CODIGO_VIDEO_AULA INT)
RETURNS VOID
AS $$
DECLARE
...
SELECT FROM PROFESSOR_DELETAR_MODULO(3)
...
/* PARAMETROS: CPF_ALUNO, CODIGO_VIDEO_AULA */
SELECT FROM ASSISTIR_VIDEO_AULA('11223344555', 19)
